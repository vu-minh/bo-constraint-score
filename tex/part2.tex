...
We will show the average value and one standard deviation of $f_{score}$.
For t-SNE and LargeVis, $f_{score}$ is evaluated for different perplexity.
For UMAP, $f_{score}$ is evaluated for different number of neighbors (\emph{n\_neighbors}) while fixing the second hyperparameter \emph{min\_dist} to its default value of 0.1.
...
$f_{score}$ increases when perplexity/\emph{n_neighbors} increases, reaches to its maximum value, and decreases when perplexity/\emph{n\_neighbors} is too large.
...
Detail clearly the strategy used (EI), the hyperparam (xi=0.1) and its purpose for exploration.

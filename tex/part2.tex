...
As shown as in our experiments, the desired structure may exist in the whole solution space but the deterministic scores like $AUC_{log}RNX$ or BIC-based score can not reveal.
...
We thus not only need a reliable quality measurement score to tune the hyperparameters but also need a flexible score which can reveal the user's desired structure.
...
Moreover it can not scale for large dataset since it has to calculate all the sets of k-nearest neighbors in HD for all values possible of $k$ from 2 to N.
It has thus a high complexity of $O(DN^2logN)$~\cite{lee2005} ($D$ is the dimension of the original data, often be large for image data, $N$ is the number of data points in a dataset), which is higher than the complexity $O(DN^2) of t-SNE.
...
\subsubsection*{The constraint generation and the visualization quality can be evaluated by user.}
+ Auto or user
\par

...
Fig.~\ref{fig:score:stability:COIL20} shows $f_{score}$ results for $t$-SNE, LargeVis and UMAP (with fixed {min\_dist} of 0.1) embeddings of \emph{COIL20}.
The results are shown for \emph{COIL20}, but are also true for the other selected datasets.
When the number of labeled instances increases, $f_{score}$ is more stable since the variance (presented by filled regions in Fig.~\ref{fig:score:stability:COIL20}) decreases.

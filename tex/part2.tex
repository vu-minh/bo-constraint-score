...
This section prepares the experimental setup for evaluating $f_{score}$ with six selected datasets, which are presented in Sec.\ref{sec:xp:data}.
The preparation of input pairwise constraints for $f_{score}$ is presented in Sec.~\ref{sec:xp:constraint}.
In order to compare the proposed $f_{score}$ with other quality metrics and to evaluate the performance of BayOpt approach, the grid of hyperparameters of all three DR methods are prepared and the optimization strategy for BayOpt is chosen, that are detailed in Sec.~\ref{sec:xp:bo}
...
% For instance, with 10 labeled points for each of the 10 classes of the DIGITS dataset, 450 similar links and 2250 dissimilar links can be generated.
The labeled points are used to indicate the relationship of belonging to the same or to different groups.
The points can also be freely chosen and grouped by the users to indicate the relationship they care about.
The generated pairwise constraints can express the information encoded in these labeled points.
$f_{score}$ function takes the constraints as input and evaluates how well the encoded information is preserved in the embeddings.
...
Through the obtained experimental results, we demonstrate firstly the behaviour of $f_{score}$ (Sec.~\ref{sec:result:fscore}) and explain how this score captures the characteristic of different embeddings.
$f_{score}$ is empirically proved to be stable (Sec.~\ref{sec:result:stability}) and comparable to $AUC_{log}RNX$ and BIC-based score for evaluating the quality of the embeddings (Sec.~\ref{sec:result:compare}).
After proving that $f_{score}$ is reliable, we can use this score under BayOpt framework to perform the task of tuning hyperparameters for the examined DR methods (Sec~\ref{sec:result:bo}).
Finally in Sec.~\ref{sec:result:flexibility}, the proposed score is shown to be flexible, that can help us to discover the users' desired structure encoded in the input constraints.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Behaviour of $f_{score}$}\label{sec:result:fscore}
...
For instance, with 10 labeled points for each of the 10 classes of DIGITS dataset, 450 similar links and 2250 dissimilar links can be generated.
...
The function of score values is not smooth as the input constraints vary, but its form suggests that finding its global maximum is feasible.
...
Given that first question, it should be noted that with a fixed number of labeled points we can generate different sets of pairwise constraints.
The second question is therefore: with different sets of constraints generated from the same fixed number of labeled points, is $f_{score}$ stable?
...
This means that, for instance, the set of labeled points in the next setting does not contain the labeled points in the previous setting.
...
On one hand, given 10 labeled points for each sub-groups, BayOpt finds 82 as the perplexity for $t$-SNE embedding that best respect the constraints.
...
% This goal can be easily encoded in term of labeled points and transformed into pairwise constraints for the proposed $f_{score}$.
